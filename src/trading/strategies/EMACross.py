...
class EMACross(Strategy):
    def __init__(self, config: EMACrossConfig):
        ...
        self._redis = None
        # Latest fast/slow EMA per instrument, published as live dashboard metrics.
    ...
    def on_bar(self, bar: Bar):
        ...
        elif fast.value < slow.value:
            if self.portfolio.is_net_long(iid):
                exit_qty = sum(
                    p.quantity.as_double()
                    for p in self.cache.positions_open(strategy_id=self.id, instrument_id=iid)
                )
                self._publish_order_event("submitted", iid, "SELL", str(exit_qty))
